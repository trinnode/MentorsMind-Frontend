...
function App() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <div
        className={`min-h-screen bg-gray-50 font-sans text-gray-900 pb-20 ${!isOnline ? "pt-10" : ""}`}
      >
        <OfflineBanner />
        {networkError && (
          <NetworkErrorToast
            ...
            <div className="hidden items-center gap-2 rounded-2xl bg-gray-50 p-1 md:flex">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    handleViewChange(item.id as AppView, item.label)
                  }
        ...
        <aside className="fixed bottom-16 left-4 z-40 hidden w-72 rounded-[1.5rem] border border-gray-100 bg-white/95 p-4 shadow-xl backdrop-blur md:block">
          ...
          <div className="mt-3 text-[11px] text-gray-500">
            Budget: {budgetStatus.jsBudgetKb}KB JS /{" "}
            {budgetStatus.chunkBudgetKb}KB chunks / {budgetStatus.imageBudgetKb}
            KB images
          </div>
        </aside>
        ...
        <footer className="fixed bottom-0 left-0 right-0 border-t border-gray-100 bg-white/80 py-4 text-center text-[10px] text-gray-400 backdrop-blur-sm">
          Demo Version 1.0 • Built with Vite, React & Tailwind CSS • Powered by
          Stellar
        </footer>
      </div>
